...
void set_game_window(ANativeWindow *window) {
    ...
    ANativeWindow *previous = nullptr;
    {
        ...
        previous = session.game_window;
        session.game_window = window;
        if (session.game_window)
            ANativeWindow_acquire(session.game_window);
    }
...
ANativeWindow *acquire_game_window() {
    auto &session = android_session_state();
    std::lock_guard<std::mutex> guard(session.game_mutex);
    if (session.game_window)
        ANativeWindow_acquire(session.game_window);
    return session.game_window;
}

bool has_game_window() {
    auto &session = android_session_state();
    std::lock_guard<std::mutex> guard(session.game_mutex);
    return session.game_window != nullptr;
}
