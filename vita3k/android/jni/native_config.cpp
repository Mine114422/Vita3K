...
void read_config_object(JNIEnv *env, jobject obj, const EmulatorConfigFields &fields,
    ...
    config.check_for_updates = env->GetBooleanField(obj, fields.checkForUpdates) != JNI_FALSE;
...
std::string read_string_field(JNIEnv *env, jobject obj, jfieldID field) {
    auto *value = reinterpret_cast<jstring>(env->GetObjectField(obj, field));
    if (!value)
        return {};
...
bool apply_runtime_config_object(JNIEnv *env, jobject obj, const EmulatorConfigFields &fields, EmuEnvState &emuenv) {
    auto &current_config = emuenv.cfg.current_config;

    // Only copy values that existing Vita3K runtime code can safely consume while a title is running.
    current_config.high_accuracy = env->GetBooleanField(obj, fields.highAccuracy) != JNI_FALSE;
    current_config.resolution_multiplier = env->GetFloatField(obj, fields.resolutionMultiplier);
    current_config.disable_surface_sync = env->GetBooleanField(obj, fields.disableSurfaceSync) != JNI_FALSE;
    current_config.screen_filter = read_string_field(env, obj, fields.screenFilter);
    current_config.v_sync = env->GetBooleanField(obj, fields.vSync) != JNI_FALSE;
    current_config.anisotropic_filtering = static_cast<int>(env->GetIntField(obj, fields.anisotropicFiltering));
    current_config.async_pipeline_compilation = env->GetBooleanField(obj, fields.asyncPipelineCompilation) != JNI_FALSE;
    current_config.export_textures = env->GetBooleanField(obj, fields.exportTextures) != JNI_FALSE;
    ...
    current_config.export_as_png = env->GetBooleanField(obj, fields.exportAsPng) != JNI_FALSE;
    current_config.fps_hack = env->GetBooleanField(obj, fields.fpsHack) != JNI_FALSE;
    emuenv.cfg.turbo_mode = env->GetBooleanField(obj, fields.turboMode) != JNI_FALSE;
    current_config.audio_volume = static_cast<int>(env->GetIntField(obj, fields.audioVolume));
    emuenv.cfg.disable_motion = env->GetBooleanField(obj, fields.disableMotion) != JNI_FALSE;
    emuenv.cfg.controller_analog_multiplier = env->GetFloatField(obj, fields.controllerAnalogMultiplier);
    read_int_array_field(env, obj, fields.controllerBinds, emuenv.cfg.controller_binds);
    read_int_array_field(env, obj, fields.controllerAxisBinds, emuenv.cfg.controller_axis_binds);
    current_config.pstv_mode = env->GetBooleanField(obj, fields.pstvMode) != JNI_FALSE;
    current_config.sys_button = static_cast<int>(env->GetIntField(obj, fields.sysButton));
    current_config.sys_lang = static_cast<int>(env->GetIntField(obj, fields.sysLang));
    current_config.sys_date_format = static_cast<int>(env->GetIntField(obj, fields.sysDateFormat));
    current_config.sys_time_format = static_cast<int>(env->GetIntField(obj, fields.sysTimeFormat));
    current_config.ime_langs = { static_cast<uint64_t>(env->GetLongField(obj, fields.imeLangs)) };
    lang::set_locale(current_config.sys_lang);
    current_config.psn_signed_in = env->GetBooleanField(obj, fields.psnSignedIn) != JNI_FALSE;
    current_config.log_active_shaders = env->GetBooleanField(obj, fields.logActiveShaders) != JNI_FALSE;
    current_config.log_uniforms = env->GetBooleanField(obj, fields.logUniforms) != JNI_FALSE;
    current_config.color_surface_debug = env->GetBooleanField(obj, fields.colorSurfaceDebug) != JNI_FALSE;
    ...
    emuenv.cfg.show_compile_shaders = env->GetBooleanField(obj, fields.showCompileShaders) != JNI_FALSE;
    emuenv.cfg.log_compat_warn = env->GetBooleanField(obj, fields.logCompatWarn) != JNI_FALSE;
    emuenv.cfg.log_level = static_cast<int>(env->GetIntField(obj, fields.logLevel));
    emuenv.cfg.performance_overlay = env->GetBooleanField(obj, fields.performanceOverlay) != JNI_FALSE;
    emuenv.cfg.performance_overlay_detail = static_cast<int>(env->GetIntField(obj, fields.performanceOverlayDetail));
    emuenv.cfg.performance_overlay_position = static_cast<int>(env->GetIntField(obj, fields.performanceOverlayPosition));
    emuenv.cfg.screenshot_format = static_cast<int>(env->GetIntField(obj, fields.screenshotFormat));

    emuenv.kernel.debugger.log_imports = env->GetBooleanField(obj, fields.logImports) != JNI_FALSE;
    emuenv.kernel.debugger.log_exports = env->GetBooleanField(obj, fields.logExports) != JNI_FALSE;
    emuenv.kernel.debugger.dump_elfs = env->GetBooleanField(obj, fields.dumpElfs) != JNI_FALSE;
    emuenv.audio.set_global_volume(current_config.audio_volume / 100.f);
...
Java_org_vita3k_emulator_NativeLib_getGlobalConfig(JNIEnv *env, jclass) {
    ...
    fill_config_object(env, config_object, fields, emuenv->cfg.current_config, emuenv->cfg, emuenv);
...
Java_org_vita3k_emulator_NativeLib_setCurrentEmulatorPath(JNIEnv *env, jclass, jstring path_str) {
    auto *emuenv = get_emuenv();
    if (!emuenv || !path_str || has_game_running())
        return JNI_FALSE;
...
Java_org_vita3k_emulator_NativeLib_saveGlobalConfig(JNIEnv *env, jclass, jobject config_obj) {
    auto *emuenv = get_emuenv();
    ...
    read_config_object(env, config_obj, fields, emuenv->cfg.current_config, emuenv->cfg, emuenv, true);
    if (emuenv->renderer)
        app::apply_renderer_config(*emuenv);
    config::save_current_config(emuenv->cfg, emuenv->config_path, {});
}
...
Java_org_vita3k_emulator_NativeLib_getCustomConfig(JNIEnv *env, jclass, jstring title_id_str) {
    ...
    const std::string title_id = jstring_to_string(env, title_id_str);
    Config::CurrentConfig current_config;
    if (!config::load_custom_config(current_config, emuenv->config_path, title_id))
        return nullptr;
    app::ensure_camera_defaults(emuenv->cfg);
    ...
    jobject config_object = env->NewObject(fields.cls, fields.ctor);
    fill_config_object(env, config_object, fields, current_config, emuenv->cfg, emuenv);
    return config_object;
}

JNIEXPORT void JNICALL
Java_org_vita3k_emulator_NativeLib_saveCustomConfig(JNIEnv *env, jclass, jstring title_id_str, jobject config_obj) {
    auto *emuenv = get_emuenv();
    if (!emuenv || !config_obj)
        return;
    ...
    read_config_object(env, config_obj, fields, current_config, config_copy);
    config::save_custom_config(current_config, emuenv->config_path, title_id);
}

JNIEXPORT jboolean JNICALL
Java_org_vita3k_emulator_NativeLib_applyRuntimeConfig(JNIEnv *env, jclass, jobject config_obj) {
    auto *emuenv = get_emuenv();
    if (!emuenv || !config_obj || !has_game_running())
        return JNI_FALSE;
    ...
    return apply_runtime_config_object(env, config_obj, fields, *emuenv) ? JNI_TRUE : JNI_FALSE;
}
...
Java_org_vita3k_emulator_NativeLib_deleteCustomConfig(JNIEnv *env, jclass, jstring title_id_str) {
    ...
    const std::string title_id = jstring_to_string(env, title_id_str);
    config::delete_custom_config(emuenv->config_path, title_id);
}
...
Java_org_vita3k_emulator_NativeLib_shouldShowCustomDriverOptions(JNIEnv *, jclass) {
    const auto device_info = renderer::enumerate_vulkan_devices();
    for (size_t index = 1; index < device_info.gpu_names.size(); ++index) {
        std::string gpu_name = device_info.gpu_names[index];
        std::transform(gpu_name.begin(), gpu_name.end(), gpu_name.begin(), [](unsigned char ch) {
            return static_cast<char>(std::tolower(ch));
        });
        if (gpu_name.find("mali") != std::string::npos)
            return JNI_FALSE;
    }
