...
void common_dialog_overlay::build_ime_dialog(DialogState &dialog) {
    ...
    const int num_btns = m_ime_cancelable ? 2 : 1;
    const uint16_t btns_total_w = static_cast<uint16_t>(num_btns * k_ime_btn_w + (num_btns - 1) * k_ime_btn_gap);
...

    {
        ...
        submit.bg.set_size(k_ime_btn_w, k_ime_field_h);
        submit.bg.back_color = { 1.f, 1.f, 1.f, 0.10f };
        submit.bg.border_radius = 6;
        submit.text.set_text(lang::get(lang::str::submit));
        ...
        submit.text.set_size(k_ime_btn_w, k_font_size_small + 4);
        submit.value = 0;
        m_buttons.push_back(std::move(submit));
        btn_x = static_cast<int16_t>(btn_x + k_ime_btn_w + k_ime_btn_gap);
    }

    if (m_ime_cancelable) {
        button_entry cancel;
        cancel.bg.set_pos(btn_x, btn_y);
        cancel.bg.set_size(k_ime_btn_w, k_ime_field_h);
        cancel.bg.back_color = { 1.f, 1.f, 1.f, 0.04f };
        cancel.bg.border_radius = 6;
        cancel.text.set_text(lang::get(lang::str::cancel));
        ...
        cancel.text.set_size(k_ime_btn_w, k_font_size_small + 4);
        cancel.value = 1;
    ...
    case pad_button::cross: {
        if (m_selected_button == 0) {
            if (dialog.active_ime) {
                ...
                memcpy(dialog.ime.result, dialog.active_ime->str.c_str(),
                    (dialog.active_ime->str.length() + 1) * sizeof(uint16_t));
