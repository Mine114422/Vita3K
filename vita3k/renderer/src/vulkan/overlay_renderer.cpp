...
bool OverlayRenderer::init(VKState &state) {
    ...
    for (uint32_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        m_desc_pools[i] = device.createDescriptorPool(pool_info);
    }
...
vk::ImageView OverlayRenderer::upload_font(vk::CommandBuffer cmd, const overlay::font *f) {
    ...
    m_staging_garbage[m_current_frame % MAX_FRAMES_IN_FLIGHT].push_back(std::move(staging));
...
vk::DescriptorSet OverlayRenderer::allocate_descriptor_set() {
    vk::DescriptorSetAllocateInfo alloc_info{
        .descriptorPool = m_desc_pools[m_current_frame % MAX_FRAMES_IN_FLIGHT],
    };
...
void OverlayRenderer::draw_command(vk::CommandBuffer cmd,
    ...
    const uint32_t frame_idx = m_current_frame % MAX_FRAMES_IN_FLIGHT;
    ...
    auto &vb = m_vertex_buffers[frame_idx];
...
void OverlayRenderer::prepare(vk::CommandBuffer cmd_buffer,
    ...
    auto &garbage = m_staging_garbage[frame_idx];
    for (auto &buf : garbage)
        buf.destroy();
    garbage.clear();

    m_state->device.resetDescriptorPool(m_desc_pools[frame_idx]);
