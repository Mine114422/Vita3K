...
bool VKState::create(std::unique_ptr<renderer::State> &state, const Config &config) {
    // Create Instance
    {
        ...
        if (!config.current_config.custom_driver_name.empty()) {
            void *vulkan_handle = android_driver::load_custom_vulkan_driver(config.current_config.custom_driver_name);
            if (vulkan_handle) {
...
renderer::VulkanDeviceInfo renderer::enumerate_vulkan_devices(const std::string &custom_driver_name) {
    ...
    try {
        ...
        if (!custom_driver_name.empty()) {
            void *vulkan_handle = android_driver::load_custom_vulkan_driver(custom_driver_name);
            if (vulkan_handle) {
                PFN_vkGetInstanceProcAddr vk_get_instance_proc_addr = reinterpret_cast<PFN_vkGetInstanceProcAddr>(dlsym(vulkan_handle, "vkGetInstanceProcAddr"));
                if (vk_get_instance_proc_addr) {
                    dispatch.init(vk_get_instance_proc_addr);
                    info.custom_driver_loaded = true;
                } else {
                    LOG_WARN("Custom driver {} loaded without vkGetInstanceProcAddr; falling back to the system Vulkan loader", custom_driver_name);
                }
            } else {
                LOG_WARN("Failed to load custom driver {}; falling back to the system Vulkan loader", custom_driver_name);
            }
        }
#endif
