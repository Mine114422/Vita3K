...
void MainWindow::on_controls_action_triggered() {
    if (!m_controls_dialog) {
        m_controls_dialog = new ControlsDialog(emuenv, this);
        connect(m_controls_dialog, &QObject::destroyed, this, [this] {
            m_controls_dialog = nullptr;
        });
    }
    m_controls_dialog->show();
    m_controls_dialog->raise();
    m_controls_dialog->activateWindow();
}
...
void MainWindow::open_settings(int tab_index) {
    if (!m_settings_dialog) {
        m_settings_dialog = new SettingsDialog(emuenv, m_gui_settings, {}, tab_index, this);
...
void MainWindow::signal_game_shutdown() {
    if (!emuenv.renderer)
        return;
...
void MainWindow::boot_game(const std::string &title_id) {
    ...
    const auto abort_boot = [&](const QString &msg) {
        ...
        if (emuenv.renderer) {
            ...
            if (runtime_initialized && renderer_initialized) {
                app::deinit(emuenv);
                app::destroy(emuenv);
        ...
        if (emuenv.backend_renderer == renderer::Backend::OpenGL && m_game_window)
...
void MainWindow::on_context_menu_requested(const QPoint &global_pos, const std::vector<const app::AppEntry *> &apps) {
    ...
    connect(&menu, &AppsListContextMenu::open_settings_requested,
        this, [this](const std::string &app_path, int tab) {
            auto *dlg = new SettingsDialog(emuenv, m_gui_settings, app_path, tab, this);
