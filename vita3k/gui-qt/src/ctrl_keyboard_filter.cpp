...
bool CtrlKeyboardFilter::eventFilter(QObject *watched, QEvent *event) {
    ...
    case QEvent::KeyRelease: {
        ...
        const auto matches = [key = ke->key()](int primary, int alternate) {
            return key == primary || key == alternate;
        };
...
void CtrlKeyboardFilter::update_state(int qt_key, bool pressed) {
    ...
    if (pressed)
        m_pressed_keys.insert(qt_key);
    else
        m_pressed_keys.erase(qt_key);

    const auto is_pressed = [this](int primary, int alternate) {
        const auto primary_it = primary != 0 ? m_pressed_keys.find(primary) : m_pressed_keys.end();
        if (primary_it != m_pressed_keys.end())
            return true;
        return alternate != 0 && m_pressed_keys.find(alternate) != m_pressed_keys.end();
    };
    ...
    const auto set_bit_if_pressed = [&](int primary, int alternate, uint32_t bit, uint32_t &mask) {
