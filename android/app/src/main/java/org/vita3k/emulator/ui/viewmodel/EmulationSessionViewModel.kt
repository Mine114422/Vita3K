...
class EmulationSessionViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun openMenu(emulator: Emulator, pauseGame: Boolean = true) {
        ...
        if (pauseGame) {
            runCatching { NativeLib.pauseGame() }
        }
        runCatching { NativeLib.setInputIntercepted(true) }
        uiState = uiState.copy(
            showMenu = true,
            isPaused = if (pauseGame) {
                true
            } else {
                runCatching { NativeLib.isGamePaused() }.getOrDefault(false)
            },
    ...
    fun closeMenu(emulator: Emulator) {
        releaseInputs(emulator)
        if (uiState.isPaused) {
            runCatching { NativeLib.resumeGame() }
        }
        runCatching { NativeLib.setInputIntercepted(false) }
        uiState = uiState.copy(
            showMenu = false,
            isPaused = false,
            isEditingControls = false,
